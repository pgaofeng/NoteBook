...
find "$directory" -type f -name "*.md" -exec bash -c '  
    ...
    # 尝试替换update-data:后面的时间  
    sed -i "/update-data:/s/\(update-data: \).*/\1$new_timestamp/" "$filename"  
  
    # 检查替换是否成功（即原始文件中是否包含这一行）  
    if ! grep -q "update-data: $new_timestamp" "$filename"; then  
        # 如果替换没有成功（即原始文件中没有这一行），则插入到第二行   
       # sed -i "/^---$/ \\update_data: $new_timestamp" "$filename"  
        sed -i "0,/^---$/ {n;i\\
update-data: $new_timestamp
        }" "$filename"
    fi  
' bash {} \;